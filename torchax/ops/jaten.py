...
@op(torch.ops.aten.copy_, is_jax_function=False, is_view_op=True, needs_env=True)
...
def repeat_interleave(self, repeats, dim=0):  # noqa: F811
...
def _embedding_jvp(padding_idx, scale_grad_by_freq, sparse, primals, tangents):
  ...
  y = _disable_pad_idx_grad_embedding_impl(
    a, w, padding_idx, scale_grad_by_freq, sparse
  )
...
def _aten_embedding(a, w, padding_idx=-1, scale_grad_by_freq=False, sparse=False):
  ...
  return _disable_pad_idx_grad_embedding_impl(
    a, w, padding_idx, scale_grad_by_freq, sparse
  )
...
def _aten__embedding_bag(
  ...
  for bag in range(num_bags):
    ...
    end = int(indices.shape[0] if bag + 1 == num_bags else offsets_np[bag + 1])
    ...
    if end - start > 0:
      if mode == 0:
        ...
        raise ValueError(
          f"mode can only be 0 (sum), 1 (mean) or 2 (max) but got {mode=}"
        )
...
def _aten_expand(x, dims):
  ...
  if len(shape) < len(dims):
    ...
    # truncate
  dims = [fix_dims(p, s) for p, s in zip(dims, shape, strict=False)]
...
def split_with_sizes(x, sizes, dim=0):
  ...
  return [
    ...
    for start, end in zip([0] + list(splits[:-1]), splits, strict=False)
  ]
...
def adaptive_avg_pool2or3d(
  ...
  if all(s % o == 0 for o, s in zip(output_size, shape[-out_dim:], strict=False)):
    ...
    kernel = tuple(
      i - (o - 1) * s
      for i, o, s in zip(shape[-out_dim:], output_size, stride, strict=False)
    )
...
def _aten_avg_pool(
  ...
  if divisor_override is not None:
    ...
    unequal_paddings = (pad[0] != pad[1] for pad in padding)
    ...
    if len(unequal_padding_indices) > 0:
      ...
      skip_indices = [x + offset for x in unequal_padding_indices]
...
def _generate_indices(dims, skip_dim_indices=None):
  if skip_dim_indices is None:
    skip_dim_indices = []
  res = []
...
def _aten_broadcast_tensors(*tensors):
  ...
  def _broadcast_dimensions(input_shape, output_shape):
    ...
    res = tuple(
      ...
      for i, (in_dim, out_dim) in enumerate(
        zip(input_shape, output_shape, strict=False)
      )
    )
    return res
  ...
  if (
    len(tensors) == 1 and len(tensors[0]) >= 1 and isinstance(tensors[0][0], jax.Array)
  ):
    tensors = tensors[0]
...
def _aten_unique_consecutive(
  ...
  if return_inverse or return_counts:
    counts = (
      jnp.append(include_indices[1:], input_tensor.shape[dim]) - include_indices[:]
    )
...
def _aten_median(self, dim=None, keepdim=False):
  ...
  if dim is None:
    ...
    index = _with_reduction_scalar(_get_median_index, self, dim, keepdim).astype(
      jnp.int64
    )
    return output, index
...
def _aten_nanmedian(input, dim=None, keepdim=False, *, out=None):
  ...
  if dim is None:
    ...
    index = _with_reduction_scalar(_get_median_index, input, dim, keepdim).astype(
      jnp.int64
    )
    return output, index
...
def _aten_upsample(
  ...
  scale = [shape[i] / image.shape[i] for i in spatial_dims]
  if scale_factors:
    scale = scale_factors
  if scales_h:
    scale[0] = scales_h
